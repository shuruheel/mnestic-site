...
scoring method, and in this case, it is set to `'tf_idf'` which takes into
consideration of global statistics when scoring documents. You can also use
`'tf'`. The resulting scores are bound to the variable `s`. Finally, the results
are ordered in descending order of score (`-s`).
